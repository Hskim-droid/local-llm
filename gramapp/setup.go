...
func setup(p profile) error {
	...
	say("  · 영상은 옆 .txt(전사문)가 있으면 그걸 씁니다.")
	say("    없으면 그때 음성 모델(약 470MB)을 받습니다. 보고서 모델과 동시에 안 올립니다.")
	say("  · PPT는 PPTX로 저장한 것만 됩니다.")
	say("  · 스캔 PDF·차트만 있는 장표는 그때 그림 모델(약 3GB)을 추가로 받습니다.")
	say("    보고서 모델과 동시에 올리지 않습니다.")
	say("")
	...
	say("")
	say("준비됐습니다.")
	say("  다음에 용도(보고서·회의록·번역)를 고르고, 파일 창이 열립니다.")
