...
1. [Ollama](https://ollama.com/download) 를 **한 번만** 설치 (트레이에 라마 아이콘). 없으면 프로그램이 사이트를 엽니다.
2. [dist/로컬LLM보고서.exe](dist/로컬LLM보고서.exe), [dist/시작.bat](dist/시작.bat), [dist/읽어주세요.txt](dist/읽어주세요.txt) 를 **같은 폴더**에 둡니다.
3. **`시작.bat` 을 더블클릭** 합니다. 검은 창의 한글 고지를 그대로 따라가면 됩니다. 창을 닫지 마세요.
4. 파일 고르는 창에서 영상·PPT·PDF를 Ctrl로 여러 개 선택합니다.
5. 끝나면 `이름_보고서\보고서.docx` 폴더가 열립니다. 원본은 그대로입니다.

아이콘 위에 파일을 **끌어다 놓아도** 됩니다.

용량: 실행 파일 약 7MB. 모델은 별도(16GB 그램 약 5GB).
