...
1. [Ollama](https://ollama.com/download) 를 **한 번만** 설치 (트레이에 라마 아이콘). 프로그램이 없으면 주소를 열어 줍니다.
2. GitHub에서 [dist/로컬LLM보고서.exe](dist/로컬LLM보고서.exe) 와 [dist/시작.bat](dist/시작.bat) 를 받아 **같은 폴더**에 둡니다.
3. **`시작.bat` 을 더블클릭** 합니다.
4. 한국어 안내대로 모델을 받습니다. 창을 닫지 마세요.
5. 파일 고르는 창이 열리면 영상·PPT·PDF를 Ctrl로 여러 개 선택합니다.
6. 끝나면 `이름_보고서\보고서.docx` 폴더가 열립니다.
